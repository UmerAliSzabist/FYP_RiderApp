...
const ProductDetails = ({ navigation, route }) => {
	...
	const UserDetails = (adId) => {
		console.log('---')
        Axios.get(`http://10.0.2.2:5000/user/userDetailsById/${adId}`)
            .then((res) => {
    ...
    const Details = (adId) => {
        Axios.get(`http://10.0.2.2:5000/user/rentedProductDetail/${adId}`)
            .then((res) => {
	...
	const handleMarkAsDelivered = () => {
        ...
        Axios.post(`http://10.0.2.2:5000/user/markasdelivered/${productDetails._id}`)
            .then((res) => {
